...
def test_highway_128(hash_tester):
    hash_tester(
        hasher_type=pyhash.highway_128,
        bytes_hash=205029416337089142837388334492957817459,
        seed_hash=[
            131606380489010322043134332560055467821,
...
def test_highway_256(hash_tester):
    hash_tester(
        hasher_type=pyhash.highway_256,
...
def bench_highway_128(benchmark, hash_bencher):
    hash_bencher(benchmark, pyhash.highway_128, 263168739977411690410017013291704716368)


@pytest.mark.benchmark(group="hash128", disable_gc=True)
def bench_highway_256(benchmark, hash_bencher):
    hash_bencher(benchmark, pyhash.highway_256, 263168739977411690410017013291704716368)
